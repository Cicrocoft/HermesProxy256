...
|---|---|---|---|
| D-1 | **SOLVED 1 Sep. The spirit healer works, and the crash that hid it was never in this path.** TBC needs a handshake the modern protocol removed: pick the healer's gossip option -> `SMSG_SPIRIT_HEALER_CONFIRM` -> client shows a prompt -> `CMSG_SPIRIT_HEALER_ACTIVATE`. That CONFIRM packet does not exist on modern builds (absent from 69110's table, WPP's V5_5_0 and TrinityCore master), so it was dropped in silence and the client gave up with `CMSG_CLOSE_INTERACTION` two seconds later. `HERMES_256_SPIRITHEALER` completes the handshake on the player's behalf - consent was already given by picking the option. It was disabled on 29 Aug because turning it on crashed the client; **that crash was D-4's opcode collision**, which any resurrect triggered regardless of how it was started - the spirit healer merely reached it first. With D-4 fixed the knob works. | **SOLVED** | live 1 Sep |
| D-2 | **SOLVED 1 Sep, in two parts, and the opcode theory in the first version of this row was wrong.** (1) `WriteCorpseData` wrote `WowGuid128.Empty` for Owner although the handler fills it; its own comment predicted the cost - "corpse ownership (loot/release UI) is untested". A corpse belonging to nobody gets no reclaim offer. `HERMES_256_CORPSEOWNER` sends the real guid; the client then sent `CMSG_RECLAIM_CORPSE` for the first time ever. (2) The arrow and the resurrect prompt need `SMSG_DEATH_RELEASE_LOC`, which had to move to 0x460181 (see D-4) and is back under `HERMES_256_DEATHLOC`. **0x460181 was inferred when D-4 landed and is now measured**: arrow, prompt and release hint all returned with no fault. Eliminated first, all already correct: the opcode table, the fragment list `[CGObject, Tag_Corpse]`, the object type, the stationary position, and the field order. | **SOLVED** | live 1 Sep |
