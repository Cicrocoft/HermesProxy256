...
| Q-1 | **SOLVED (located and verified) 29 Aug — `QuestCompleted` IS an `ActivePlayerData` field, carried in the ActivePlayer create.** Earlier in the same session this row said the opposite; that exclusion was **wrong**, and the reason is worth keeping: the field is a **dynamic array**, and `clientfields-ActivePlayerData.json` was walked over OUR OWN block where the count is 0 — so the element loop was never entered and the field could not appear. A walk over a block that does not contain a thing cannot prove the thing does not exist. **Wire format, read off Blizzard's bytes:** inside `SMSG_UPDATE_OBJECT`'s ActivePlayer create, `u32 count` followed by `count x u64`; live count = **64** words (512 B). **Verified three independent ways:** (1) the bits decode through `QuestV2.UniqueBitFlag` to 16 real quest ids — 170, **179 Dwarven Outfitters**, 182, 183, 218, 233, 234, 282, 400, 420, 2160, 3112, 3361, 3364, 3365, 5541 — a coherent Coldridge Valley history for the character; (2) quest 179's bit is exactly where the client's own bit test says it must be (`UniqueBitFlag 188` -> word `(188-1)>>6 = 2`, bit `187&63 = 59`, and word 2 = `0x8800000008008000` has bit 59 set); (3) the **same 512 bytes appear byte-identical in two independent captures of the same character** (`ap_rowine.bin` @9174 and `ap_w16.bin` @9211), and in both they sit **exactly 2149 bytes before the end of the descriptor**. **The proxy needs no new logic** — `CompletedQuestTracker` already indexes `(questBit-1)>>6 / &63`, the store is persisted, and `UpdateHandler.cs:392` already fills `ActivePlayerData.QuestCompleted`. | **located + verified; emit position not yet pinned** | `ground-truth/qcompleted_rowine.bin` is the array; `w16.pkt` packet [67] |
| Q-1d | **The emit position is NOT pinnable by distance-from-end — measured and refuted — and the region it belongs to is itself misaligned.** `HERMES_256_APDOFFSETS=1` (new, default off, in `ModernDescriptors.cs`) logs the APD block start/end and every dynamic count's wire offset. Live: `start=3569 end=10129 len=6560`, the 24 counts span **fromEnd 1035..1127**. Blizzard's QuestCompleted count is at **fromEnd 1641**, so **none of ours is it** — the landmark was a stated hypothesis and it is now dead. Cause: our APD is 6560 B where Blizzard's is ~7720; the 1160 B gap is not just the 516 B array, so distance-from-end cannot transfer between blocks with different populated arrays. **Worse, and this is the real blocker:** walking the client's own APD reader (0x713E50) over our CURRENT block at the right visibility (base 3570, flags **7**, `rowine_pd.py`'s full Emu setup) shows the reader consumes **6288 B where we emit 6559**, and **only 1 of our 24 counts coincides with any read at all** — and that one is a `u8`, i.e. coincidence. So our count chain does not sit where the reader reads counts. That misalignment predates this work and must be resolved before the array can be placed; inserting 516 bytes into a region already off by 271 would be guessing in a linear unmasked block. **Note also `clientfields-ActivePlayerData.json` is cached at `visibility_flags: 1` while the live block is 0x07** — the same compare-like-with-like trap as `gate256.py`'s hardcoded `flags=1`; that cache cannot be used for this. | **open — blocked on APD tail alignment, not on the quest field** | live `APD-OFFSETS` line; reader walk |
...
| Q-1c | **Capture #16 adds a sixth live ActivePlayer create — it is NOT the first, and an earlier claim in this row said otherwise.** `tools-256-spike/` already held `ap_rowine.bin` (11323 B), `ap_live8/9`, `ap_s1` and `ap_mememe07`; `run256.sh`'s VEJB comment names `ap_rowine.bin` as "the live 0x07 create, validated". P0-FREEZE's "needs a live ActivePlayer create - none exists yet" is therefore **stale**, and repeating it instead of running `ls tools-256-spike/ap_*.bin` is exactly the absence-claim failure CLAUDE.md warns about. What #16 genuinely adds: a **different character** (Gnome warrior, level 8, quest history) carved as `ap_w16.bin` (11360 B), so the APD tail can be diffed across two live characters rather than measured on one. Also `ground-truth/w16_s2.bin`, `w16.pkt`, `w16_parsed.txt`, `updates_16.bin`. First boundary measured: **UnitData at flags=7 consumes 904 bytes**. The PlayerData boundary is NOT settled — `rowine_pd.py`'s working setup (base 922, flags 7, `r9=7`, `rbp:0x1ed0=1`) aborts on this block, so its length is character-dependent and needs its own pass. | **asset landed, tail work started not finished** | key in 73 s; 446/446 decrypt; stream tiles 131086/131086; `carve_ap.py` sanity: race 7 class 1 level 8 hp 183 |
